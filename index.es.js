...
export function ApiService(options) {
  ...
  this._opts = (Array.isArray(options) ? options : [options]).map(opt => normalizeOptions(opt));
  this.queue = [];
 ...
 */
const createTree = (base, items) => items.reduce((res, item) => {
  const opts = getOpts(item);
  if (!base._isApiService) {
    opts.adapter = base;
  }
  const next = base._isApiService ? base.extend(opts) : new ApiService(opts);
...
const getFrom = services => {
  const get = (name, payload) => payload !== undefined ? services[name].doRequest(payload) : services[name];
...
const createObjectTree = (base, items) => Object.keys(items).reduce((res, key) => {
  res[key] = base._isApiService ? base.extend(items[key]) : new ApiService(items[key]);
  return res;
}, {});
...
export const rest = (name, payload = defaultRest) => Array.isArray(payload) ? payload.map(key => generateRestItem[key](name)) : Object.keys(payload).map(key => Object.assign(generateRestItem[key](name), payload[key]));
